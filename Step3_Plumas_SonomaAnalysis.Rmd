...
plumas <- fread("Routput/Plumas/Plumas_in_prescription.csv")
sonoma <- fread("Routput/Sonoma/sonoma_in_prescription.csv")
...
###
# Plumas vs. Sonoma
p <- plumas %>% group_by(long, lat, dates, season, year) %>% mutate(n = n()) %>% ungroup() %>% group_by(LIFE_FORM, year, season, long, lat)  %>% summarise(ndays = sum(n)) %>% ungroup() %>% group_by(LIFE_FORM, year, season) %>% summarise(pmean = mean(ndays)) %>% ungroup()
s <- sonoma %>% group_by(long, lat, dates, season, year) %>% mutate(n = n()) %>% ungroup() %>% group_by(LIFE_FORM, year, season, long, lat)  %>% summarise(ndays = sum(n)) %>% ungroup() %>% group_by(LIFE_FORM, year, season) %>% summarise(smean = mean(ndays)) %>% ungroup()

ps <- full_join(p, s)
ps <- na.omit(ps)

ggplot(subset(ps, LIFE_FORM %in% "CONIFER"),aes(x = year)) + geom_line(aes(y = pmean), col = 'brown') + geom_line(aes(y = smean), col = 'black') +  facet_wrap(vars(season))
